...
/*! \namespace osc - Optix Siggraph Course */
  ...
  struct CameraFrame {
                        ...
                        /* up-vector        : */const gdt::vec3f &up)
    {
      ...
      frame.vz
        = (interest==origin)
        ? gdt::vec3f(0,0,1)
        : /* negative because we use NEGATIZE z axis */ - gdt::normalize(interest - origin);
      ...
      //   ? gdt::vec3f(0,1,0)
      //   : gdt::normalize(cross(up,frame.vz));
      frame.vy = gdt::normalize(cross(frame.vz,frame.vx));
    ...
    void forceUpFrame()
    {
      ...
      if (fabsf(dot(frame.vz,upVector)) < 1e-6f)
        ...
        return;
      frame.vx = gdt::normalize(cross(upVector,frame.vz));
      frame.vy = gdt::normalize(cross(frame.vz,frame.vx));
      modified = true;
    }
  ...
  struct CameraFrameManip {
    ...
    // /*! this gets called when the user presses a key on the keyboard ... */
    // virtual void special(int key, const gdt::vec2i &where) { };
    ...
    // virtual void mouseButtonLeft  (const gdt::vec2i &where, bool pressed) {}
      
    // /*! mouse button got either pressed or released at given location */
    // virtual void mouseButtonMiddle(const gdt::vec2i &where, bool pressed) {}
      
    // /*! mouse button got either pressed or released at given location */
    // virtual void mouseButtonRight (const gdt::vec2i &where, bool pressed) {}
  ...
  struct GLFCameraWindow : public GLFWindow {
    ...
    // /*! mouse got dragged with left button pressedn, by 'delta'
    //   pixels, at last position where */
    // virtual void mouseDragLeft  (const gdt::vec2i &where, const gdt::vec2i &delta) {}

    // /*! mouse got dragged with left button pressedn, by 'delta'
    //   pixels, at last position where */
    // virtual void mouseDragRight (const gdt::vec2i &where, const gdt::vec2i &delta) {}

    // /*! mouse got dragged with left button pressedn, by 'delta'
    //   pixels, at last position where */
    // virtual void mouseDragMiddle(const gdt::vec2i &where, const gdt::vec2i &delta) {}
